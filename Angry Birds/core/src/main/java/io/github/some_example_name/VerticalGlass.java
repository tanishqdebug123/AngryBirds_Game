...
public class VerticalGlass extends Actor{
    ...
    public VerticalGlass(World world, float x, float y, float width, float height, float angle){
        this.world = world;
        this.width=width;
        this.height=height;
        this.initialAngle=angle;

        wb2_texture=new Texture("verticalg.png");
        setWidth(width);
        setHeight(height);
    ...
    public void draw(Batch batch, float parentAlpha) {
        batch.draw(wb2_texture,
            getX(), getY(),width / 2, height / 2, width, height, 1f, 1f, (float) Math.toDegrees(body.getAngle()), 0, 0, wb2_texture.getWidth(), wb2_texture.getHeight(), false, false);
    }
