...
public class level1 implements Screen {
    ...
    public level1(Main game) {
        ...
        level1_struct l1_st = new level1_struct(world, stage);
...
//        createWall(0, 0, 800, 1);
    ...
    public void render(float delta) {
        ...
        for (LargePig pigs: list_largepigs) {
            if(pigs.getHitCount()==3) {
                pigs.remove();
                world.destroyBody(pigs.body);
                birds_destroyed++;
            }
            else if(pigs.getHitCount()<3){
                pigs.increasehitcount();
            }
        }
        ...
        if(birds_destroyed==total_birds){
            ...
            game.setScreen(new level1complete(game));
        }
